...
TEST(ParallelFlatHashMap, IteratesMsan) {
  ...
  std::vector<phmap::parallel_flat_hash_map<int, balast>> garbage;
  for (int i = 0; i < 100; ++i) {
    phmap::parallel_flat_hash_map<int, balast> t;
    for (int j = 0; j < 100; ++j) {
      t[j];
...
TEST(ParallelFlatHashMap, MergeExtractInsert) {
  ...
  phmap::parallel_flat_hash_map<int, int> m = {{1, 7}, {2, 9}};
...
TEST(ParallelFlatHashMap, Any) {
  phmap::parallel_flat_hash_map<int, std::any> m;
  m.emplace(1, 7);
  auto it = m.find(1);
  ASSERT_NE(it, m.end());
  ...
  phmap::parallel_flat_hash_map<std::any, int, H, E> m2;
  m2.emplace(1, 7);
  auto it2 = m2.find(1);
