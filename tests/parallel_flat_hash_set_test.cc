...
template <class T>
using Set =
    phmap::parallel_flat_hash_set<T, StatefulTestingHash, StatefulTestingEqual, Alloc<T>>;
...
TEST(ParallelFlatHashSet, BitfieldArgument) {
  ...
  n = 0;
  phmap::parallel_flat_hash_set<int> s = {n};
  s.insert(n);
  s.insert(s.end(), n);
  s.insert({n});
  s.erase(n);
...
TEST(ParallelFlatHashSet, MergeExtractInsert) {
  ...
  phmap::parallel_flat_hash_set<std::unique_ptr<int>, Hash, Eq> set1, set2;
