...
static const uint64_t prime_sizes[] = 
{
    1ull,
    3ull,
    7ull,
    13ull,
    17ull,                  // prime close to 2 ** 4
    31ull,                  // prime close to 2 ** 5
    67ull,                  // prime close to 2 ** 6
    127ull,                 // prime close to 2 ** 7
    257ull,                 // prime close to 2 ** 8
    521ull,                 // prime close to 2 ** 9
    1031ull,                // prime close to 2 ** 10
    2053ull,                // prime close to 2 ** 11
    4099ull,                // prime close to 2 ** 12
    8191ull,                // prime close to 2 ** 13
    16411ull,               // prime close to 2 ** 14
    32771ull,               // prime close to 2 ** 15
    65537ull,               // prime close to 2 ** 16
    131071ull,              // prime close to 2 ** 17
    262147ull,              // prime close to 2 ** 18
    524287ull,              // prime close to 2 ** 19
    1048583ull,             // prime close to 2 ** 20
    2097169ull,             // prime close to 2 ** 21
    4194319ull,             // prime close to 2 ** 22
    8388617ull,             // prime close to 2 ** 23
    16777259ull,            // prime close to 2 ** 24
    33554467ull,            // prime close to 2 ** 25
    67108879ull,            // prime close to 2 ** 26
    134217757ull,           // prime close to 2 ** 27
    268435459ull,           // prime close to 2 ** 28
    536870923ull,           // prime close to 2 ** 29
    1073741827ull,          // prime close to 2 ** 30
    2147483647ull,          // prime close to 2 ** 31
    4294967311ull,          // prime close to 2 ** 32
    8589934609ull,          // prime close to 2 ** 33
    17179869209ull,         // prime close to 2 ** 34
    34359738421ull,         // prime close to 2 ** 35
    68719476767ull,         // prime close to 2 ** 36
    137438953481ull,        // prime close to 2 ** 37
    274877906951ull,        // prime close to 2 ** 38
    549755813911ull,        // prime close to 2 ** 39
    1099511627791ull,       // prime close to 2 ** 40
    2199023255579ull,       // prime close to 2 ** 41
    4398046511119ull,       // prime close to 2 ** 42
    8796093022237ull,       // prime close to 2 ** 43
    17592186044423ull,      // prime close to 2 ** 44
    35184372088891ull,      // prime close to 2 ** 45
    70368744177679ull,      // prime close to 2 ** 46
    140737488355333ull,     // prime close to 2 ** 47
    281474976710677ull,     // prime close to 2 ** 48
    562949953421381ull,     // prime close to 2 ** 49
    1125899906842679ull,    // prime close to 2 ** 50
    2251799813685269ull,    // prime close to 2 ** 51
    4503599627370517ull,    // prime close to 2 ** 52
    9007199254740997ull,    // prime close to 2 ** 53
    18014398509482143ull,   // prime close to 2 ** 54
    36028797018963971ull,   // prime close to 2 ** 55
    72057594037928017ull,   // prime close to 2 ** 56
    144115188075855881ull,  // prime close to 2 ** 57
    288230376151711813ull,  // prime close to 2 ** 58
    576460752303423619ull,  // prime close to 2 ** 59
    1152921504606847009ull, // prime close to 2 ** 60
    2305843009213693951ull, // prime close to 2 ** 61
    4611686018427388039ull  // prime close to 2 ** 62
};


/**
 * from https://github.com/lemire/fastrange (Daniel Lemire)
 *
...
public:
    probe_seq(size_t hash, size_t mask) {
        ...
        mask_ = mask; // + 1;
        offset_ = mask_ ? hash % mask_ : 0;
    }
    size_t offset() const { return offset_; }
    size_t offset(size_t i) const { return (offset_ + i) < mask_ ? (offset_ + i) : (offset_ + i) % mask_; }

    void next() {
        index_ += Width;
        offset_ += index_;
        offset_ = offset_ < mask_ ? offset_ : offset_ % mask_;
    }
...
inline bool IsValidCapacity(size_t ) { return true; } // ((n + 1) & n) == 0 && n > 0; }
...
private:
...
#if 0
        const constexpr size_t sz = 128;
        size_t hashes[sz];
        for (size_t i = 0; i != old_capacity; ) 
        {
            const size_t cnt = std::min(sz, old_capacity-i);
            const size_t to = i + cnt;
            for (size_t j=i; j<to; ++j)
            {
                if (IsFull(old_ctrl[j]))
                    hashes[j-i] = PolicyTraits::apply(HashElement{hash_ref()},
                                                      PolicyTraits::element(old_slots + j));
                else
                    hashes[j-i] = 0xa1a1a1a1a1a1a1a1ull;
                
            }
            for (size_t j=i; j<to; ++j)
            {
                size_t hash = hashes[j-i];
                if (hash == 0xa1a1a1a1a1a1a1a1ull)
                    continue;
                auto seq = probe(hash);
                if (seq.offset() >= old_capacity)
                    continue;
                auto target = find_first_non_full(hash);
                size_t new_i = target.offset;
                set_ctrl(new_i, H2(hash));
                PolicyTraits::transfer(&alloc_ref(), slots_ + new_i, old_slots + j);
            }
#if 1
            for (size_t j=i; j<to; ++j)
            {
...
#else
        //h1_shift = (h1_shift == 7) ? 5 : 7;
        // h1_shift = (h1_shift == 5) ? 9 : (h1_shift - 2);
    ...
    void rehash_and_grow_if_necessary() {
        if (capacity_ == 0) {
            ...
            const size_t num_primes = sizeof(prime_sizes) / sizeof(uint64_t);
            size_t i=0;
            for (; i<num_primes && prime_sizes[i] <= capacity_; ++i);
            resize(prime_sizes[i]);
        }
    }
...
protected:
    ...
    size_t prepare_insert(size_t hash) PHMAP_ATTRIBUTE_NOINLINE {
        if (PHMAP_PREDICT_FALSE((capacity_ == 0)))
